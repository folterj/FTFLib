...
FtfImage::FtfImage()
{
	imageDatas = gcnew List<FtfImageData^>();
	clear();
}
...
bool FtfImage::readImage(String^ filename)
{
	bool hashOk;
	...
	if (reader.open(filename))
	{
		...
		for (int elementi = 0; elementi < elements; elementi++)
		{
			...
			if (value)
			{
				if (Enum::TryParse(value, true, colormodel))
				{
			...
			if (value)
			{
				if (Enum::TryParse(value, true, componentformat))
				{
			...
			if (value)
			{
				if (Enum::TryParse(value, true, colorChannelsFormat))
				{
					switch (colorChannelsFormat)
					{
						case FtfColorChannelsFormat::I:		fileFormat->colorChannelsFormat = ColorChannelsFormat::I;		break;
						case FtfColorChannelsFormat::IA:	fileFormat->colorChannelsFormat = ColorChannelsFormat::IA;		break;
						case FtfColorChannelsFormat::AI:	fileFormat->colorChannelsFormat = ColorChannelsFormat::AI;		break;
						case FtfColorChannelsFormat::RGB:	fileFormat->colorChannelsFormat = ColorChannelsFormat::RGB;		break;
						case FtfColorChannelsFormat::BGR:	fileFormat->colorChannelsFormat = ColorChannelsFormat::BGR;		break;
						case FtfColorChannelsFormat::ARGB:	fileFormat->colorChannelsFormat = ColorChannelsFormat::ARGB;	break;
						case FtfColorChannelsFormat::RGBA:	fileFormat->colorChannelsFormat = ColorChannelsFormat::RGBA;	break;
						case FtfColorChannelsFormat::BGRA:	fileFormat->colorChannelsFormat = ColorChannelsFormat::BGRA;	break;
						default:							fileFormat->colorChannelsFormat = ColorChannelsFormat::None;	break;
					}
				}
				else
				{
...
bool FtfImage::writeImage(String^ filename)
{
	return writeImage(filename, nullptr);
}

bool FtfImage::writeImage(String^ filename, ChannelFormat^ outputFormat)
{
	bool ok = false;
	...
	FtfColorChannelsFormat colorChannelsFormat;
	int elements;
	...
	if (writer.open(filename, gcnew SHA512Managed()))
	{
		for (int elementi = 0; elementi < elements; elementi++)
		{
			...
			{
				case ColorChannelsFormat::I:	colorChannelsFormat = FtfColorChannelsFormat::I;	break;
				case ColorChannelsFormat::IA:	colorChannelsFormat = FtfColorChannelsFormat::IA;	break;
				case ColorChannelsFormat::AI:	colorChannelsFormat = FtfColorChannelsFormat::AI;	break;
				case ColorChannelsFormat::RGB:	colorChannelsFormat = FtfColorChannelsFormat::RGB;	break;
				case ColorChannelsFormat::BGR:	colorChannelsFormat = FtfColorChannelsFormat::BGR;	break;
				case ColorChannelsFormat::ARGB:	colorChannelsFormat = FtfColorChannelsFormat::ARGB;	break;
				case ColorChannelsFormat::RGBA:	colorChannelsFormat = FtfColorChannelsFormat::RGBA;	break;
				case ColorChannelsFormat::BGRA:	colorChannelsFormat = FtfColorChannelsFormat::BGRA;	break;
				default:						colorChannelsFormat = FtfColorChannelsFormat::NONE;	break;
			}
			...
			// dimensions
			writer.setElementTag(FtfTag::dimensionsLabel,
								String::Format("{0} {1} {2}", imageData->height, imageData->width, channelFormat->channels));
...
String^ FtfImage::getErrorMessage()
{
	return errorMessage;
}
...
int FtfImage::getIntTag(FtfFileReader^ reader, String^ label, bool required)
{
	int x;
...
array<Byte>^ FtfImage::bufferToBytes(FtfImageData^ imageData, ChannelFormat^ channelFormat, bool floatGammaCorrection, bool reverse)
{
	array<Byte>^ byteData;
	...
	byteData = gcnew array<Byte>(height * width * channelFormat->bytesPerPixel);
	pixelData = gcnew array<Byte>(bytesPerPixel);
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			if (grayscaleMode)
			{
				...
				switch (colorChannelsFormat)
				{
					case ColorChannelsFormat::IA:	c1 = i;	c2 = a;	break;
					case ColorChannelsFormat::AI:	c1 = a;	c2 = i;	break;
					default:			/* I */		c1 = i;			break;
				}
			}
			else
			{
				...
				switch (colorChannelsFormat)
				{
					case ColorChannelsFormat::BGR:	c1 = b;	c2 = g;	c3 = r;			break;
					case ColorChannelsFormat::ARGB:	c1 = a;	c2 = r;	c3 = g;	c4 = b;	break;
					case ColorChannelsFormat::RGBA:	c1 = r;	c2 = g;	c3 = b;	c4 = a;	break;
					case ColorChannelsFormat::BGRA:	c1 = b;	c2 = g;	c3 = r;	c4 = a;	break;
					default:			/* RGB */	c1 = r;	c2 = g;	c3 = b;			break;
				}
			}
			pos0 = 0;
...
Bitmap^ FtfImage::bytesToBitmap(array<Byte>^ byteData, int width, int height, ChannelFormat^ channelFormat)
{
	Bitmap^ bitmap;

	if (width != 0 && height != 0)
	{
		bitmap = gcnew Bitmap(width, height, channelFormat->pixelFormat);
		BitmapData^ bitmapData = bitmap->LockBits(Rectangle(0, 0, bitmap->Width, bitmap->Height), ImageLockMode::WriteOnly, bitmap->PixelFormat);
		int len = bitmapData->Stride * bitmapData->Height;
		Marshal::Copy(byteData, 0, bitmapData->Scan0, len);
...
void FtfImage::bytesToBuffer(array<Byte>^ byteData, FtfImageData^ imageData, bool floatGammaCorrection, bool reverse)
{
	array<Byte>^ pixelData;
	double i, r, g, b;
	double a = 1;
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			if (grayscaleMode)
			{
				...
				switch (colorChannelsFormat)
				{
					case ColorChannelsFormat::IA:	i = c1;	a = c2;	break;
					case ColorChannelsFormat::AI:	a = c1;	i = c2;	break;
					default:			/* I */		i = c1;			break;
				}
			...
			else
			{
				...
				switch (colorChannelsFormat)
				{
					case ColorChannelsFormat::BGR:	b = c1;	g = c2;	r = c3;			break;
					case ColorChannelsFormat::ARGB:	a = c1;	r = c2;	g = c3;	b = c4;	break;
					case ColorChannelsFormat::RGBA:	r = c1;	g = c2;	b = c3;	a = c4;	break;
					case ColorChannelsFormat::BGRA:	b = c1;	g = c2;	r = c3;	a = c4;	break;
					default:			/* RGB */	r = c1;	g = c2;	b = c3;			break;
				}
